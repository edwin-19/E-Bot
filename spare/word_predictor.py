...
class WordPredictor(object):
    ...
    def __init__(self, order =2, case_sensitive =True, vocab_size = 100000):
    ...
    def learn_from_text(self,text):
        ...
        #If first of text, previous is the empty string
        ...
        for word in self._tokenize_phrase(text):
            ...
            #Add history for n previous words, 0 < n <= oder of Markov Chain
            for order  in reversed(range(0, len(hash_deq))):
                #Calcualte state hash
    ...
    def predict(self,text):
        """Predict a number of following word candidates based on given text
         Arguments:
